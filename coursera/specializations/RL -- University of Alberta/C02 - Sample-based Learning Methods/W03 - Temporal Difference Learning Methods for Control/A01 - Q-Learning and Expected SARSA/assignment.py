...
class ExpectedSarsaAgent(BaseAgent):
    ...
    def agent_step(self, reward, observation):
        ...
        # your code here
        # pi (a | s) = {
        #     1 - epsilon + (epsilon / num_actions);    a == action,
        #     (epsilon / num_actions)              ;    a != action
        # }
        pi = [(self.epsilon / self.num_actions) if a != action else (1 - self.epsilon) + (self.epsilon / self.num_actions) for a in range(self.num_actions)]
        expected_return = np.sum([p * q for p, q in zip(pi, current_q)])
        self.q[self.prev_state, self.prev_action] += (self.step_size * (reward + (self.discount * expected_return) - self.q[self.prev_state, self.prev_action]))
