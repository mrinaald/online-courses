...
def get_gradient(s, weights):
    ...
    grads[1]['b'] = 1
    grads[1]['W'] = x.T
    grads[0]['b'] = np.multiply(weights[1]['W'].T, np.where(x > 0, 1, 0))
    grads[0]['W'] = my_matmul(s.T, np.multiply(weights[1]['W'].T, np.where(x > 0, 1, 0)))
...
class TDAgent(BaseAgent):
    ...
    def agent_step(self, reward, state):
        ...
        # ----------------
        # your code here
        state_oh = one_hot(state, self.num_states)
        last_state_oh = one_hot(self.last_state, self.num_states)
        delta = reward + (self.discount_factor * get_value(state_oh, self.weights)) - get_value(last_state_oh, self.weights)
        ...
        # ----------------
        # your code here
        grads = get_gradient(last_state_oh, self.weights)
    ...
    def agent_end(self, reward):
        ...
        # your code here
        last_state_oh = one_hot(self.last_state, self.num_states)
        delta = reward - get_value(last_state_oh, self.weights)
