...
class SarsaAgent(BaseAgent):
    ...
    def select_action(self, tiles):
        ...
        if np.random.random() < self.epsilon:
            ...
            return chosen_action, np.sum(self.w[chosen_action, tiles])
        
        action_values = [np.sum(self.w[a, tiles]) for a in range(self.num_actions)]
    ...
    def agent_step(self, reward, state):
        ...
        # `grads` Not needed, as now self.w is updated along these indices
        #grads = np.zeros_like(self.w)
        #grads[self.last_action, self.previous_tiles] = 1
        expected_value = action_value
        previous_value = np.sum(self.w[self.last_action, self.previous_tiles])
        self.w[self.last_action, self.previous_tiles] += self.alpha * (reward + (self.gamma * expected_value) - previous_value)
