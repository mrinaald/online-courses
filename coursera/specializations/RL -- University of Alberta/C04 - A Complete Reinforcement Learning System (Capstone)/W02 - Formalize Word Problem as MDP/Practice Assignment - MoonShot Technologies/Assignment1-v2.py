...
class LunarLanderEnvironment(environment.BaseEnvironment):
    ...
    def env_step(self, action):
        ...
        # your code here
        def is_crashed():
            if pos_y == land_y and pos_x == land_x:
                if vel_y > 3 or vel_x > 10:
                    return True
                if 5 < angle and angle < 355:
                    return True
            if fuel <= 0 and pos_y != land_y:
                return True
            if pos_y < land_y:
                return True
            if pos_y == land_y and pos_x != land_x:
        ...
        angle_reward = - (angle if angle <= 180 else 360 - angle)
        reward = fuel - abs(land_x - pos_x) - abs(land_y - pos_y) + angle_reward
        
        if pos_x == land_x and pos_y == land_y:
            terminal = True
            reward = fuel
        if is_crashed():
            terminal = True
