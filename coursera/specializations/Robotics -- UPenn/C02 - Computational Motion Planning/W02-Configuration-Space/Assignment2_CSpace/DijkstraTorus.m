...
function updateNeighbor(x, y, min_dist, current)
  neighbor = sub2ind(size(map), x, y);
  if ((map(neighbor) == 1) || (map(neighbor) == 4) || (map(neighbor) == 6))
    if min_dist + 1 < distances(neighbor)
...
while true
    ...
    if i - 1 > 0
      ...
      updateNeighbor(i - 1, j, min_dist, current);
    else
      updateNeighbor(nrows, j, min_dist, current);
    end;
    ...
    if i + 1 <= nrows
      ...
      updateNeighbor(1, j, min_dist, current);
    end;

    if j - 1 > 0
      % Left neighbor exists
      updateNeighbor(i, j - 1, min_dist, current);
    else
      updateNeighbor(i, ncols, min_dist, current);
    end;
    ...
    if j + 1 <= ncols
      ...
      updateNeighbor(i, 1, min_dist, current);
    end;
