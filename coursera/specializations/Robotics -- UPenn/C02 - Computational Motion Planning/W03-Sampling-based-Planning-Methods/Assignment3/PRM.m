...
for i = 2:nsamples
    ...
    %
    [sorted_distances, sorted_samples] = sort(distances);

    for s=1:min(k, i-1)
      if LocalPlanner(x, samples(:, sorted_samples(s)))
        nedges = nedges + 1;
        % edges(nedges, 1) = sorted_samples(s);
        % edges(nedges, 2) = i;
        edges(nedges, 1) = i;
        edges(nedges, 2) = sorted_samples(s);
        edge_lengths(nedges) = sorted_distances(s);
      end
    end
