...
%%%% BEGIN: INITIALIZATION CODE FOR TRACKER HERE %%%%
klt_tracker = vision.PointTracker();
initialize(klt_tracker, img_pts_init, cell2mat(images(1)));
%%%% END: INITIALIZATION CODE FOR TRACKER HERE %%%%
...
for i = 2:size(images,1)
    %%%% BEGIN: CODE FOR TRACKING HERE %%%%
    % Store corners and visualize results (if desired)
    [img_pts, ~] = klt_tracker(cell2mat(images(i)));
    %%%% END: CODE FOR TRACKING HERE %%%%
    corners(:,:,i) = img_pts;
end
