...
function myPose = particleLocalization_rtheta(ranges, scanAngles, map, param)
...
function corr = computeCorrelation(map, startIndex, endIndex)
    if startIndex(1) < 1 || startIndex(1) >= numCols || startIndex(2) < 1 || startIndex(2) >= numRows || map(startIndex(2), startIndex(1)) > 0.0
        %corr = L * -10;
        corr = 1e-6;
        return;
    end
