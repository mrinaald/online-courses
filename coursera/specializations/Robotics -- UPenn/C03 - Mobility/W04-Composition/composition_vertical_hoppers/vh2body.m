...
%   
    ...
    axes(handles.axes4);
    set(handles.axes4, 'Units', 'pixels', 'Position', [700, 200, 450, 200]);
    cla();
    plot(t, X(:,1))
    title('Hybrid Mode');
% 
...
%   ylabel('x')
  ...
  %axes(handles.axes2);
  axes(handles.axes3);
  set(handles.axes3, 'Units', 'pixels', 'Position', [700, 450, 450, 200]);
  cla();
  hold on
  plot(t, -z,'-')
  plot([0,t(end)],p.rho*[1,1],'k--')
...
%   hold on
   ...
   %axes(handles.axes1);
   set(handles.axes1, 'Units', 'pixels', 'Position', [200, 200, 400, 400]);
  %{
  % animate
  framedel = 0.01;
  slowmo = 0.5;
