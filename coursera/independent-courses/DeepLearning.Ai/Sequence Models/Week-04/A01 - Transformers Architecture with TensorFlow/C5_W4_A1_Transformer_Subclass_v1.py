...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    if mask is not None: # Don't replace this None
        scaled_attention_logits += ((1.0 - mask) * -1.0e9)
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        self_mha_output = self.mha(x, x, x, attention_mask=mask, training=training, return_attention_scores=False)  # Self attention (batch_size, input_seq_len, fully_connected_dim)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        mult_attn_out1, attn_weights_block1 = self.mha1(x, x, x, attention_mask=look_ahead_mask, training=training, return_attention_scores=True)  # (batch_size, target_seq_len, d_model)
        ...
        mult_attn_out2, attn_weights_block2 = self.mha2(Q1, enc_output, enc_output, attention_mask=padding_mask, training=training, return_attention_scores=True)  # (batch_size, target_seq_len, d_model)
