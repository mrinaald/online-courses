...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    # Helper function to compute projection
    def compute_projection(a, b):
        return (np.dot(a, b) / (np.linalg.norm(b) ** 2)) * b

    # Step 3: Compute the projections of mu over the bias axis and the orthogonal axis (≈ 2 lines)
    mu_B = compute_projection(mu, bias_axis)
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = compute_projection(e_w1, bias_axis)
    e_w2B = compute_projection(e_w2, bias_axis)
    
    # Helper function to compute corrected vectors
    def compute_corrected_vector(e, e_B, mu_B, mu_orth):
        return np.sqrt(1 - (np.linalg.norm(mu_orth) ** 2)) * (e_B - mu_B) / np.linalg.norm(e - mu_B)
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = compute_corrected_vector(e_w1, e_w1B, mu_B, mu_orth)
    corrected_e_w2B = compute_corrected_vector(e_w2, e_w2B, mu_B, mu_orth)
